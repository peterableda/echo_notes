...
class Settings:
    ...
    def __init__(self):
        ...
        # API Configuration resolution order: API_KEY -> WHISPER_API_KEY -> /tmp/jwt
    ...
    def whisper_headers(self) -> dict:
        """Headers for Whisper API requests."""
        return {"Authorization": f"Bearer {self.api_key}"}

    @property
    def llm_headers(self) -> dict:
        """Headers for LLM API requests."""
        return {"Authorization": f"Bearer {self.api_key}"}

    def get_project_dir(self, name: str, timestamp: str) -> Path:
        """Project directory path for a transcription."""
    ...
    def _load_quick_actions(self) -> List[Dict[str, str]]:
        """Load configurable Quick Actions: file -> env -> defaults."""
